...
with first_col1:
    cumulative_note = (
        f"較股票池等權基準 {cumulative_excess:+.2%}"
...
with first_col4:
    drawdown_note = (
        f"與股票池等權基準差異 {drawdown_difference:+.2%}"
...
st.warning(
    "AI Top 5 的歷史累積報酬雖高於股票池等權基準，但最大回撤超過 50%，"
        ...
        if benchmark_return_column is not None:
            fold_figure.add_trace(
                go.Bar(
                    ...
                    marker_color="#2563eb",
                    hovertemplate=(
                        "Fold：%{x}<br>股票池等權基準報酬：%{y:.2%}<extra></extra>"
                    ),
                )
            )
